...
variable is present and mutually matching, so deploying the workflow code to
Production with the workflow **disabled** is safe (the existing MassageBook
links stay live). Remaining work is entirely operator-side: set Production
...
3. **Frontend flag — RESOLVED** (`Bookings.jsx`/`SquareBooking.jsx`,
   ...
   any other value keeps the MassageBook fallback. The old flag is ignored. All
...
| `VITE_ENABLE_BOOKING_REQUESTS` | PUBLIC, frontend build | Yes | Exactly `true` renders the request flow; any other value shows MassageBook fallback. Must be set in **Preview** (item 14) before any Preview redeploy; set `true` in Production only with the workflow enabled (resolved B3) |
...
- The new booking flow (`SquareBooking.jsx`) renders only when
  `VITE_ENABLE_BOOKING_REQUESTS === "true"`; any other value keeps the
  MassageBook fallback (`Booking.jsx`). The legacy `VITE_ENABLE_SQUARE_SANDBOX`
...
- Roll back by restoring a prior deployment in Vercel (git revert on `main` then
  ...
  endpoints 503 and the site falls back to MassageBook links — no behavior
  regression for public visitors.
- The DB migrations are additive; no destructive data. Booking requests created
  under the feature are retained; none block MassageBook because holds only cover
  active (pending/approving) rows.
...
- **Preview env:** `VITE_ENABLE_BOOKING_REQUESTS=true` must be added to the
  ...
  without it the Preview shows the MassageBook fallback.
