...
import { addOnsDurationMinutes, addOnsForKeys, addOnsPriceCents, normalizeAddOnKeys } from "./add-ons.js";
 ...
 * from the client). Marketing consent is separate, optional and never required;
 * a false or absent value never creates a subscription.
 */
...
export function validateBookingRequest(body) {
  ...
  const addOnKeys = normalizeAddOnKeys(body.addons ?? body.addOnKeys);
  ...
  if (!addOnKeys) return { error: "Unknown add-on" };
  if (startError) return { error: startError };

  const config = getServiceConfig(serviceKey);
  const durationMinutes = totalDurationMinutes(config, addOnKeys);
  ...
  return {
    data: {
      ...
      durationMinutes,
      price: totalPriceCents(config, addOnKeys) / 100,
      priceCents: totalPriceCents(config, addOnKeys),
      addOnKeys,
      addOns: addOnsForKeys(addOnKeys).map((addOn) => ({
        key: addOn.key,
        name: addOn.name || "Facial Massage",
        itemName: addOn.itemName,
        variationName: addOn.variationName,
...
function squareBookingDurationMinutes(booking) {
  ...
  const duration = segments.reduce((sum, segment) => sum + Number(segment?.durationMinutes || 0), 0);
  return Number.isFinite(duration) && duration > 0 ? duration : null;
}

export function totalDurationMinutes(service, addOnKeys = []) {
  return service.durationMinutes + addOnsDurationMinutes(addOnKeys);
}

export function totalPriceCents(service, addOnKeys = []) {
  return service.price * 100 + addOnsPriceCents(addOnKeys);
}

export function serviceWithAddOns(service, addOnKeys = []) {
  return {
    ...service,
    durationMinutes: totalDurationMinutes(service, addOnKeys),
  };
}

export function squareVersionForCreate(version) {
...
export async function buildAppointmentSegments(client, { service, addOnKeys = [], teamMemberId, primaryServiceVariationVersion = null }) {
  let version = primaryServiceVariationVersion;
  if (typeof version !== "bigint" && typeof version !== "number") {
    const catalogObject = await client.catalog.object.get({ objectId: service.serviceVariationId });
    version = catalogObject?.object?.version ?? null;
  }
  if (typeof version !== "bigint" && typeof version !== "number") throw new Error("service_variation_version_missing");
  const segments = [{
  ...
  for (const addOn of addOnsForKeys(addOnKeys)) {
    if (!addOn.serviceVariationId) throw new Error("add_on_variation_missing");
    const addOnObject = await client.catalog.object.get({ objectId: addOn.serviceVariationId });
    const addOnVersion = addOnObject?.object?.version ?? null;
    if (typeof addOnVersion !== "bigint" && typeof addOnVersion !== "number") throw new Error("add_on_variation_version_missing");
    segments.push({
      durationMinutes: addOn.durationMinutes,
      serviceVariationId: addOn.serviceVariationId,
      teamMemberId,
      serviceVariationVersion: squareVersionForCreate(addOnVersion),
    });
  }
  return segments;
}
...
export async function findSlotAvailability(client, { locationId, teamMemberId, service, start }) {
  ...
  for (const addOn of addOnsForKeys(service.addOnKeys || [])) {
