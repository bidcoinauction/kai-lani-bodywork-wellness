...
export function buildSquareWebhookQueueMessage(event) {
  ...
  const startAt = requiredDateString(booking.startAt, "booking_start_at");
  const locationId = safeOptionalId(booking.locationId || event.location_id, "location_id");
  if (!locationId) throw new SquareWebhookMessageError("location_id_missing");
  if (!Array.isArray(booking.appointmentSegments) || booking.appointmentSegments.length === 0) {
    throw new SquareWebhookMessageError("appointment_segments_missing");
  }
  if (booking.appointmentSegments.length > MAX_APPOINTMENT_SEGMENTS) {
  ...
  const message = {
    ...
    locationId,
    appointmentSegments: booking.appointmentSegments.map(normalizeSegment),
  };
