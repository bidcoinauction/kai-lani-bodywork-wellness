...
async function postResend(payload, idempotencyKey, metadata) {
  if (!canSend()) {
    console.info(`Email notification type=${metadata.type} bookingSuffix=${metadata.bookingSuffix} status=disabled`);
    return "disabled";
  }
  ...
  try {
    ...
    console.info(`Email notification type=${metadata.type} bookingSuffix=${metadata.bookingSuffix} status=${status}`);
    return status;
  } catch {
    console.info(`Email notification type=${metadata.type} bookingSuffix=${metadata.bookingSuffix} status=failed`);
    return "failed";
  } finally {
