...
const PUBLIC_EMAIL = "appointments@kailanibodywork.com";
...
function addOnLines(data) {
  return Array.isArray(data.addOns) ? data.addOns : [];
}

function addOnsHtml(data) {
  const addOns = addOnLines(data);
  if (addOns.length === 0) return "";
  return `<dt>Add-on</dt><dd>${addOns.map((addOn) => `${escapeHtml(addOn.name)} — ${escapeHtml(addOn.durationMinutes)} min`).join("<br>")}</dd>`;
}

function addOnsTextLines(data) {
  return addOnLines(data).map((addOn) => `Add-on: ${addOn.name} — ${addOn.durationMinutes} min`);
}
...
<html>
  <body style="font-family: Arial, sans-serif; color: #1f2d2f; line-height: 1.5;">
    ...
    <dl>
      ...
      <dt>Email</dt><dd>${escapeHtml(PUBLIC_EMAIL)}</dd>
    </dl>
