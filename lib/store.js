...
function mapRow(row) {
  if (!row) return null;
  const addOnKeys = Array.isArray(row.add_on_keys)
    ? row.add_on_keys
    : typeof row.add_on_keys === "string"
      ? JSON.parse(row.add_on_keys)
      : [];
  return {
    id: row.id,
    ...
    try {
      const result = await this.poolClient.query(
          `INSERT INTO booking_requests
           (request_key, service_key, first_name, last_name, email, phone, add_on_keys,
