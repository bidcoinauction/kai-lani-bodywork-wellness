...
function safeHttpStatus(value) {
  return Number.isInteger(value) && value >= MIN_HTTP_STATUS && value <= MAX_HTTP_STATUS ? value : null;
}
  ...
  async publishSquareWebhook(message) {
    ...
    try {
      const response = await this.fetchImpl(
        `${APPROVED_QSTASH_URL}/v2/publish/${this.workerUrl}`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        const error = new Error("qstash_publish_failed");
