...
test("rejects invalid phone numbers", async () => {
  for (const phone of ["123", "abcdefghij", "98055501001", "(980) 555-0100 x2"]) {
...
test("reuses the same idempotency key across duplicate submissions", async () => {
  installFullConfig();
  const createLog = [];
  const mock = makeEchoAvailabilityMock(createLog);
  ...
  assert.equal(createLog.length, 2);
  assert.equal(createLog[0].idempotencyKey, "idem-test-0001");
  assert.equal(createLog[1].idempotencyKey, "idem-test-0001");
});
